...
_Bool SENSOR_ProcessData = 0;
_Bool SENSOR_SendInfo = 0;

DHT22_Sensor_t miDHT_Sensor;


char cmdReceiveBuffer[10] = {0};
...
void SERIAL_RXProcessCommand(void){
    if(UART1_RxAvailable()){
        ...
        if(c != '\r' && c != '\n'){
            ...
            cmdReceiveBuffer[cmdIndex] = '\0';

            if(strcmp(cmdReceiveBuffer, "Start") == 0){
                SENSOR_SendInfo = 1;

                TMR0_ovrFlag = 0;
                SENSOR_ProcessData = 0;
                RTCC_Count = 0;
...
void SERIAL_TXProcess(void){
    if(SENSOR_ProcessData && SENSOR_SendInfo){
