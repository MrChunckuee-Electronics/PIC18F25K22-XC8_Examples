...
void LCDNokia5110_Initialize(LCDNokia5110_t lcd){
    ...
    (*(Nokia5110.TRIS) &= ~(1 << (Nokia5110.RST)));
    (*(Nokia5110.TRIS) &= ~(1 << (Nokia5110.CE)));
    (*(Nokia5110.TRIS) &= ~(1 << (Nokia5110.DC)));
    (*(Nokia5110.TRIS) &= ~(1 << (Nokia5110.DIN)));
    (*(Nokia5110.TRIS) &= ~(1 << (Nokia5110.CLK)));

    (*(Nokia5110.PORT) |= (1 << (Nokia5110.CE)));

    (*(Nokia5110.PORT) &= ~(1 << (Nokia5110.RST)));
    __nop(); __nop();
    (*(Nokia5110.PORT) |= (1 << (Nokia5110.RST)));

    LCDNokia5110_Write(0, 0x21);
    LCDNokia5110_Write(0, 0xB0);
    LCDNokia5110_Write(0, 0x04);
    LCDNokia5110_Write(0, 0x14);
    LCDNokia5110_Write(0, 0x20);
    LCDNokia5110_Write(0, 0x0C);
}
# 73 "LCDNokia5110.c"
void LCDNokia5110_Write(uint8_t mode, uint8_t data){
    if(mode == 1)
        (*(Nokia5110.PORT) |= (1 << (Nokia5110.DC)));
    else
        (*(Nokia5110.PORT) &= ~(1 << (Nokia5110.DC)));
...
void LCDNokia5110_WriteSPI(uint8_t outData){
    for(uint8_t i = 0; i < 8; i++){
        (*(Nokia5110.PORT) &= ~(1 << (Nokia5110.CLK)));
        if ((outData & 0x80) == 0)
            (*(Nokia5110.PORT) &= ~(1 << (Nokia5110.DIN)));
        else
            (*(Nokia5110.PORT) |= (1 << (Nokia5110.DIN)));
        (*(Nokia5110.PORT) |= (1 << (Nokia5110.CLK)));
        outData <<= 1;
...
void LCDNokia5110_Clear(void){
    for(int index = 0; index < (84 * 48 / 8); index++)
        LCDNokia5110_Write(1, 0x00);
    LCDNokia5110_GotoXY(0, 0);
}
# 129 "LCDNokia5110.c"
void LCDNokia5110_GotoXY(uint8_t x, uint8_t y){
    LCDNokia5110_Write(0, 0x80 | x);
    LCDNokia5110_Write(0, 0x40 | y);
}
# 145 "LCDNokia5110.c"
void LCDNokia5110_putC(char data){
    LCDNokia5110_Write(1, 0x00);
    for (int index = 0; index < 5; index++)
        LCDNokia5110_Write(1, FONT_NORMAL[data - 0x20][index]);
...
void LCDNokia5110_Image(const char IMAGE[], uint8_t x, uint8_t y){
    LCDNokia5110_GotoXY(x, y);
    for (int index = 0; index < (84 * 48 / 8); index++)
        LCDNokia5110_Write(1, IMAGE[index]);
}
# 193 "LCDNokia5110.c"
void LCDNokia5110_Bitmap(const char BITMAP[], uint8_t x, uint8_t y, uint8_t width, uint8_t height){
    for (uint8_t p = 0; p < (height / 8); p++){
        ...
        for (uint8_t i = 0; i < width; i++){
            LCDNokia5110_Write(1, BITMAP[i + (p * width)]);
        }
    }
}
