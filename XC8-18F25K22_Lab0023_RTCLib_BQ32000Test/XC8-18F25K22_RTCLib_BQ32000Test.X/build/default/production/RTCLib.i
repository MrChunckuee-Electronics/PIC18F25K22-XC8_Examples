...
# 51 "./RTCLib.h"
    ...
    _Bool RTC_GetTime(Time_t *result);
    void RTC_SetDate(const Date_t *date);
    _Bool RTC_GetDate(Date_t *result);
# 37 "RTCLib.c" 2
...
void RTC_Initialize(RTC_Model_t model, uint8_t i2cAddress){
    ...
    _rtcI2cAddress = i2cAddress & 0xFE;
...
void RTC_StartClock(void){
    ...
    if(_selectedModel == RTC_MODEL_BQ32000){
        ...
        tempReg = RTC_ReadByte(0x00);
        tempReg |= 0x80;
        RTC_WriteByte(0x00, tempReg);
        tempReg &= 0x7F;
        RTC_WriteByte(0x00, tempReg);
    }
...
# 104 "RTCLib.c"
static _Bool RTC_WaitReady(void){
    uint16_t timeout = 500;
    I2C2_Start();

    while(I2C2_Send(_rtcI2cAddress | 0x00) == 1 && timeout > 0) {
        I2C2_Start();
        timeout--;
    }
...
# 218 "RTCLib.c"
__attribute__((inline)) uint8_t DEC_to_BCD(uint8_t decimal){

    return (decimal / 10 * 16) | (decimal % 10);
}
# 232 "RTCLib.c"
__attribute__((inline)) uint8_t BCD_to_DEC(uint8_t bcd){
...
void RTC_SetTime(const Time_t *time){
    ...
    if (_selectedModel != RTC_MODEL_BQ32000){
        if (time->timeMode == 1) {
            HourBcd |= 0x60;
        } else if (time->timeMode == 0) {
            HourBcd |= 0x40;
        }
    }
    dataBuff[2] = HourBcd;

    RTC_WriteBytes(0x00, dataBuff, 3);
}
# 278 "RTCLib.c"
_Bool RTC_GetTime(Time_t *result) {
    uint8_t dataBuff[3];

    RTC_ReadBytes(0x00, dataBuff, 3);
    ...
    if (_selectedModel != RTC_MODEL_BQ32000 && (dataBuff[2] & 0x40)) {
        ...
        result->timeMode = (dataBuff[2] & 0x20) ? 1 : 0;
    } else {
...
void RTC_SetDate(const Date_t *date){
    ...
    RTC_WriteBytes(0x03, dataBuff, 4);
}
# 335 "RTCLib.c"
_Bool RTC_GetDate(Date_t *result) {
    uint8_t dataBuff[4];

    RTC_ReadBytes(0x03, dataBuff, 4);
